...
export const useUserStore = defineStore('user', {
    ...
    actions: {
        async backendLogin({ username, password }) {
            ...
            const base = await apiLogin({ username, password })
            ...
            // 后端字段：userAccount, userRole（0普通/1教师/2管理员）
            const roleNum = user.userRole
            const roleMap = { 2: 'admin', 1: 'teacher', 0: 'student' }
        ...
        async backendRegister({ username, password, confirmPassword }) {
            const base = await apiRegister({ username, password, confirmPassword })
            if (!base || typeof base.code === 'undefined') {
